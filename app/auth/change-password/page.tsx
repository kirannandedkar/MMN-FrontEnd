...
const ChangePasswordPage: React.FC = () => {
  ...
  const validate = (
    formState: IChangePasswordRequest | null,
    value: string | undefined | null,
    fieldName: string,
    formData: IChangePasswordRequest | null
  ) => {
    ...
    if (fieldName === "newPassword" || fieldName === "confirmPassword") {
      if (
        ...
        updated.confirmPassword =
          "New password and confirmed password not matched";
    }
    return updated;
  };
      ...
      <MMNContainer className="gap-[40px] pb-[40px] lg:flex-row flex-col">
        <div className="flex flex-col gap-[10px]">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  onInputChangeHandler(e.target.value, "currentPassword")
                }
              />
              {errorState?.currentPassword && (
                <ErrorMessage msg={`${errorState.currentPassword}`} />
              )}
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-2 gap-[10px] line-height-mmn-large">
            <div>
              ...
              {errorState?.newPassword && (
                <ErrorMessage msg={`${errorState.newPassword}`} />
              )}
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-2 gap-[10px] line-height-mmn-large">
          ...
          <div
            ...
            onClick={isButtonClicked ? () => {} : submitHandler}
          >
            <MMNButton
